...
def prepare_model_settings(label_count, sample_rate, clip_duration_ms,
  ...
  if length_minus_window < 0:
    ...
    spectrogram_length = 1 + tf.dtypes.cast(int(length_minus_window / window_stride_samples),dtype=tf.dtypes.float64)
  if preprocess == 'average':
    fft_bin_count = 1 + (_next_power_of_two(window_size_samples) / 2)
    average_window_width = tf.dtypes.cast(int(math.floor(fft_bin_count / feature_bin_count)),dtype=tf.dtypes.float64)
    fingerprint_width = tf.dtypes.cast(int(math.ceil(fft_bin_count / average_window_width)),dtype=tf.dtypes.float64)
...
def create_single_fc_model(fingerprint_input, model_settings, is_training):
  ...
  """
  if is_training:
    dropout_prob = tf.random.normal([1],0.0,stddev=0.01)
...
def create_conv_model(fingerprint_input, model_settings, is_training):
  ...
  """
  if is_training:
    dropout_prob = tf.random.normal([1],0.0,stddev=0.01,dtype=tf.dtypes.float64)
  ...
  first_weights = tf.Variable(
                      tf.random.normal(
                         ...
                         mean=0.0,stddev=0.01,dtype=tf.dtypes.float64),
                      name='first_weights')
  
  first_bias = tf.Variable(
                  tf.zeros([first_filter_count]),
                  name='first_bias')
  first_bias = tf.dtypes.cast(first_bias,tf.float64)
  ...
  second_bias = tf.Variable(
                  tf.zeros([second_filter_count]),
                  name='second_bias'
                  )
  second_bias = tf.dtypes.cast(second_bias,tf.float64)
  ...
  final_fc_bias = tf.Variable(
                    ...
                    name='final_fc_bias')
  final_fc_bias = tf.dtypes.cast(final_fc_bias,tf.float64)
